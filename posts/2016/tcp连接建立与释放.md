...
由此可见，由于服务端状态会一直变化，所以窗口的大小会一直在变化，通过这种变化，制约了发送端的数据发送。

由此可见，如果客户端一直没收到ack，那么最终窗口便会占满，如把10和11都发送了，但是5-11都没收到ack，此时客户端就无法送数据了。  
![tcp19](./images/tcp19.png)

没法发送数据了，客户端怎么知道服务端已经调整了窗口大小，让它可以发送数据了呢？  
