...
```
这里我们先不关心`Object.create`的细节，只需要知道它是创建一个对象（上述代码中的newMan对象），并且将其prototype指向source对象（上述代码中的man）。

`hasOwnProperty`用于判断指定的属性是否是对象自身拥有的属性。可见到`man`自身拥有`height`和`和drink`属性，而`newMan`却不拥有这两个属性（都返回了false）。

可是当`newMan`调用`drink`方法时却是可以调用通的，这就是`prototype`的作用。`newMan`在自身查找不到这些属性于是便去`prototype`中查找，而`prototype`引用了`man`对象，于是便去`man`中查找，在`man`中查找到了这两个属性。

我们可以通过代码验证下：
```javascript
newMan.__proto__ === man // true
```
__proto__可以认为就是js提供的给外部访问的prototype。

如果man中也查找不到会怎样？  
man中查找不到则会去man的prototype中继续查找，如此一直查找下去。   
所有普通的prototype链最终都指向了内置的`Object.prototype`，这个Object.prototype包含了一些js中的通用功能。例如上述中的`hasOwnProperty`便是从Object.prototype来的
```javascript
...
2. 这个临时对象的protptype关联到了Human.prototype，此处没有额外的prototype引用，因此该prototype指向了原型链顶端的Object.prototype
3. 将该临时对象绑定到Human函数的this
4. Human函数没有返回值，所以便将该临时对象作为函数返回值

我们看到这整个过程中，没有任何所谓的初始化类的过程，甚至也没有所谓的复制行为，即没有出现将任何属性和方法复制到一个对象中这种行为，javascript不需要这种方式。它用prototype，采用原型链的方式让多个对象相互关联。
