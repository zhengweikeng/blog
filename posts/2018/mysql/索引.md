...
非聚集索引的叶子节点中存储的不是“行指针”，而是主键值，并以此作为指向行的“指针”。  

![index](../images/index1.jpeg)

可见非聚集索引中，首先存储了非聚集索引的列，还有主键列，通过主键列便可以找到数据行。

如果使用非聚集索引查询，而查询列中包含了其他该索引没有覆盖的列，那么他还要进行第二次的查询，查询节点上对应的数据行的数据，这时便会采用主键列查询到对应的数据行。
...
也就是说`WHERE`或者`JOIN`中对这些前导列指定了常量，可以让`order by`不满足最左前缀规则
